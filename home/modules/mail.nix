...
let
  ...
  accounts = builtins.listToAttrs (builtins.map
    ...
    (import "${inputs.private}/_email.nix").accounts);
  debugAccounts = pkgs.runCommand "pretty-json" {} ''
    echo '${builtins.toJSON accounts}' | ${pkgs.jq}/bin/jq '.' > $out
  '';

  nextcloud = (import "${inputs.private}/_contacts.nix").nextcloud // {
    local = {
      type = "filesystem";
      fileExt = ".vcf";
...

{
  ...
  sops.secrets = mailSecrets // (import "${inputs.private}/_contacts.nix").nextcloudSops;
